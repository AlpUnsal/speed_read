...
(function() {
    ...
    const icons = {
        ...
        palette: `<svg viewBox="0 0 24 24"><circle cx="7" cy="7" r="2.5" fill="none" stroke="currentColor" stroke-width="1.5"/><circle cx="17" cy="7" r="2.5" fill="none" stroke="currentColor" stroke-width="1.5"/><circle cx="7" cy="17" r="2.5" fill="none" stroke="currentColor" stroke-width="1.5"/><circle cx="17" cy="17" r="2.5" fill="none" stroke="currentColor" stroke-width="1.5"/></svg>`
    };
    ...
    function createOverlay(article) {
            ...
            <div class="axilo-speed-control" id="axilo-speed-panel">
                ...
                <div class="axilo-setting-row">
                    <div class="axilo-setting-row-horizontal">
                        ...
                        <button class="axilo-icon-btn" id="axilo-theme-palette-btn" style="padding:0px; width:24px; height:24px;">
                            ${icons.palette}
                        </button>
                    </div>
                    <select class="axilo-setting-select" id="axilo-theme-select">
                        ...
                        <option value="sage" class="axilo-pastel-theme" style="display:none;">Sage</option>
                        <option value="iceBlue" class="axilo-pastel-theme" style="display:none;">Ice Blue</option>
                        <option value="cherry" class="axilo-pastel-theme" style="display:none;">Light Cherry</option>
                        <option value="lilac" class="axilo-pastel-theme" style="display:none;">Lilac</option>
                    </select>
                </div>
                <!-- Theme / Focus Lines -->
                <div class="axilo-setting-row-horizontal">
        ...
        chrome.storage.sync.get(['axilo_wpm', 'axilo_focus_lines', 'axilo_font_family', 'axilo_font_size', 'axilo_theme', 'axilo_hint_seen'], (result) => {
            ...
            const paletteBtn = shadow.getElementById('axilo-theme-palette-btn');
            
            function applyTheme(themeName) {
                // Remove all possible theme classes
                wrapper.classList.remove('axilo-theme-black', 'axilo-theme-grey', 'axilo-theme-white', 'axilo-theme-sage', 'axilo-theme-iceBlue', 'axilo-theme-cherry', 'axilo-theme-lilac');
                if (themeName !== 'cream') wrapper.classList.add(`axilo-theme-${themeName}`);
            }
            if (result && result.axilo_theme) {
                if (['sage', 'iceBlue', 'cherry', 'lilac'].includes(result.axilo_theme)) {
                    shadow.querySelectorAll('.axilo-pastel-theme').forEach(el => el.style.display = 'block');
                    if (paletteBtn) paletteBtn.style.color = 'var(--axilo-red)';
                }
            ...
            if (paletteBtn) {
                paletteBtn.addEventListener('click', (e) => {
                    e.stopPropagation();
                    const pastelOptions = shadow.querySelectorAll('.axilo-pastel-theme');
                    const isHidden = pastelOptions[0].style.display === 'none';
                    pastelOptions.forEach(opt => opt.style.display = isHidden ? 'block' : 'none');
                    paletteBtn.style.color = isHidden ? 'var(--axilo-red)' : 'var(--axilo-icon-color)';
                });
            }
